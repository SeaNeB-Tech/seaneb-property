...
import {
  getAreas,
  getBusinessesByArea,
  getLocationCities as getCities,
  getCountries,
  getStates,
    ...
    const load = async () => {
      try {
        ...
        if (resolvedApiStateSlug) {
          cities = await getCities(resolvedCountry.apiSlug, resolvedApiStateSlug);
        ...
        if (!selectedCity) {
          for (const state of states) {
            const stateCities = await getCities(resolvedCountry.apiSlug, state.slug);
